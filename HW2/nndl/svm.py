...
class SVM(object):
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    A_yx = A_x[y,np.arange(A_x.shape[1])]
    ones = np.ones(A_x.shape)
    
    M = A_x - A_yx + ones
    
    M_loss = M
    M_loss[y, np.arange(A_x.shape[1])] = 0
    M_loss[ M_loss < 0] = 0
    ...
    grad = np.zeros(A_x.shape)
    
    M_grad = M
    M_grad[M_grad<0] = 0
    M_grad[M_grad>0] = 1
    M_grad[y, np.arange(0, A_x.shape[1])] = 0
    
    M_grad[y, np.arange(0, A_x.shape[1])] -= np.sum(M_grad, axis = 0)
    
    grad = np.dot(M_grad, X)
